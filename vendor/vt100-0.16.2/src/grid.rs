...
impl Grid {
    ...
    pub fn set_size(&mut self, size: Size) {
        ...
        // Reflow the primary transcript. Full-screen alternate buffers retain
        // their cell coordinates, as remote applications expect after NAWS.
    ...
    pub fn reported_cursor_position(&self) -> (u16, u16) {
        (
            ...
            self.pos.col.min(self.size.cols - 1),
        )
    }
    ...
    pub fn insert_cells(&mut self, count: u16) {
        ...
        let wide = pos.col < size.cols
            && self
                ...
                .is_wide_continuation();
        let row = self.current_row_mut();
        // ECMA-48 ICH shifts within this line. Characters beyond its right
        // edge are discarded; never allocate/work in proportion to a remote
        // parameter such as CSI 65535 @ on an 80-column terminal.
    ...
    pub fn insert_lines(&mut self, count: u16) {
        ...
        for _ in 0..count.min(self.scroll_bottom - self.pos.row + 1) {
    ...
    pub fn scroll_up(&mut self, count: u16) {
        for _ in 0..count.min(self.scroll_bottom - self.scroll_top + 1) {
            ...
            if self.scrollback_len > 0 && !self.scroll_region_active() {
                ...
                while self.scrollback.len()
                    > self
                        .scrollback_len
                        .min(1_000_000 / usize::from(self.size.cols))
                {
    ...
    pub fn set_scroll_region(&mut self, top: u16, bottom: u16) {
        ...
        self.pos.row = if self.origin_mode { self.scroll_top } else { 0 };
        self.pos.col = 0;
    }
    ...
    pub fn col_wrap(&mut self, width: u16, wrap: bool) {
        if self.pos.col > self.size.cols.saturating_sub(width) {
            ...
            // Mark before scrolling: in a one-row viewport this row moves
            // straight into history and must retain its soft-wrap boundary.
            self.drawing_row_mut(prev_pos.row).unwrap().wrap(wrap);
            self.pos.col = 0;
            let scrolled = self.row_inc_scroll(1);
            // A one-row terminal scrolls the previous row into history.
            // There is then no previous drawing row to mark as wrapped.
            if scrolled > prev_pos.row {
                return;
            }
...
impl Grid {
    fn reflow(&mut self, size: Size) {
        ...
        let cursor_row = old_history + usize::from(self.pos.row);
        let saved_row = old_history + usize::from(self.saved_pos.row);
        let mut old: Vec<_> = self
            .scrollback
            .drain(..)
            .chain(self.rows.drain(..))
            .collect();
        // Empty viewport padding is not transcript text. Keep cells occupied
        // by either cursor even when the underlying line is empty.
        let used = old
            .iter()
            .rposition(|row| row.used_columns() > 0 || row.wrapped())
            .unwrap_or(0)
            .max(cursor_row)
            .max(saved_row)
            + 1;
        old.truncate(used);
        let mut result: Vec<crate::row::Row> = Vec::new();
        let mut line: Vec<crate::Cell> = Vec::new();
        let mut cursor_offset = None;
        let mut saved_offset = None;
        let mut cursor = (0usize, 0u16);
        let mut saved = (0usize, 0u16);
        for (row_index, row) in old.iter().enumerate() {
            let start = line.len();
            let mut columns = if row.wrapped() {
                usize::from(self.size.cols)
            } else {
                row.used_columns()
            };
            if row_index == cursor_row {
                cursor_offset = Some(start + usize::from(self.pos.col));
                columns = columns.max(usize::from(self.pos.col));
            }
            if row_index == saved_row {
                saved_offset = Some(start + usize::from(self.saved_pos.col));
                columns = columns.max(usize::from(self.saved_pos.col));
            }
            for col in 0..columns.min(usize::from(self.size.cols)) {
                line.push(
                    row.get(col as u16)
                        .cloned()
                        .unwrap_or_else(crate::Cell::new),
                );
            }
            if row.wrapped() && row_index + 1 < old.len() {
                continue;
            }
            let mut new_row = crate::row::Row::new(size.cols);
            let mut col = 0u16;
            let mut index = 0usize;
            while index < line.len() {
                let cell = &line[index];
                let old_width = if cell.is_wide() { 2 } else { 1 };
                let width = old_width.min(size.cols);
                if col + width > size.cols {
                    new_row.wrap(true);
                    result.push(new_row);
                    new_row = crate::row::Row::new(size.cols);
                    col = 0;
                }
                if cursor_offset == Some(index) {
                    cursor = (result.len(), col);
                }
                if saved_offset == Some(index) {
                    saved = (result.len(), col);
                }
                let mut cell = cell.clone();
                if old_width > width {
                    cell.set('\u{fffd}', *cell.attrs());
                }
                *new_row.get_mut(col).unwrap() = cell;
                if width == 2 {
                    let continuation = new_row.get_mut(col + 1).unwrap();
                    continuation.set_wide_continuation(true);
                }
                col += width;
                index += usize::from(old_width);
                if cursor_offset == Some(index) {
                    cursor = (result.len(), col);
                }
                if saved_offset == Some(index) {
        ...
        let history = result.len().saturating_sub(usize::from(size.rows));
        self.scrollback = result.drain(..history).collect();
        self.rows = result;
        self.rows
            .resize_with(usize::from(size.rows), || crate::row::Row::new(size.cols));
        let cap = self.scrollback_len.min(1_000_000 / usize::from(size.cols));
