...
impl<CB: crate::callbacks::Callbacks> vte::Perform for WrappedScreen<CB> {
    ...
    fn execute(&mut self, b: u8) {
        ...
        match b {
            ...
            12 => self.screen.ff(),
            13 => self.screen.cr(),
            14 => self.screen.shift(1),
            15 => self.screen.shift(0),
    ...
    fn esc_dispatch(&mut self, intermediates: &[u8], _ignore: bool, b: u8) {
        ...
        if let [slot @ b'('..=b'+'] = intermediates {
            self.screen.designate(*slot, b);
            return;
        }
        if intermediates == b"#" && b == b'8' {
    ...
    fn csi_dispatch(&mut self, params: &vte::Params, intermediates: &[u8], _ignore: bool, c: char) {
        ...
        match intermediates.first() {
            None => match c {
                ...
                'I' => self.screen.tab_forward(canonicalize_params_1(params, 1)),
                'Z' => self.screen.tab_backward(canonicalize_params_1(params, 1)),
                'g' => self.screen.tab_clear(canonicalize_params_1(params, 0)),
                's' => self.screen.decsc(),
                'u' => self.screen.decrc(),
                'h' => self.screen.ansi_mode(params, true),
                'l' => self.screen.ansi_mode(params, false),
