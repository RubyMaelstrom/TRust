...
impl Row {
    ...
    pub(crate) fn used_columns(&self) -> usize {
        self.cells
            .iter()
            .rposition(|cell| {
                cell.has_contents()
                    || cell.is_wide_continuation()
                    || cell.attrs() != &crate::attrs::Attrs::default()
            })
    ...
    pub fn resize(&mut self, len: u16, cell: crate::Cell) {
        ...
        self.wrapped = false;
        // A resize may bisect a wide glyph. Never leave a lead without its
        // continuation: subsequent overwrite/erase assumes the pair exists.
    ...
    pub fn clear_wide(&mut self, col: u16) {
        ...
        let other = if cell.is_wide() {
            ...
            col.checked_sub(1)
                .and_then(|col| self.cells.get_mut(usize::from(col)))
        } else {
            return;
        };
