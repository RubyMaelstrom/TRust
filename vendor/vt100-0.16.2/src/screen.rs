...
pub struct Screen {
    ...
    saved_charsets: ([u8; 4], usize),
    tab_stops: Vec<bool>,
    cursor_style: u16,
    last_printed: Option<crate::grid::Pos>,
}
...
impl Screen {
    pub(crate) fn new(size: crate::grid::Size, scrollback_len: usize) -> Self {
        ...
        Self {
            ...
            charsets: [b'B'; 4],
            active_charset: 0,
            saved_charsets: ([b'B'; 4], 0),
            tab_stops: (0..size.cols.max(1))
                .map(|col| col > 0 && col % 8 == 0)
                .collect(),
            cursor_style: 0,
            last_printed: None,
        }
    ...
    pub fn set_size(&mut self, rows: u16, cols: u16) {
        ...
        for col in old..usize::from(cols) {
    ...
    /// Number of retained rows preceding the live screen.
    pub fn history_len(&self) -> usize {
        self.grid().history_len()
    }

    /// DEC screen-wide reverse video (DECSCNM), independent of cell SGR 7.
    pub fn reverse_video(&self) -> bool {
        self.modes & MODE_REVERSE_VIDEO != 0
    }

    /// Discard retained history without changing the live screen.
    pub fn clear_scrollback(&mut self) {
        self.grid_mut().clear_history();
    }

    /// Next programmable horizontal tab stop, or the right margin.
    pub fn tab_stop_after(&self, column: u16) -> u16 {
        let cols = self.grid().size().cols;
        (column.saturating_add(1)..cols)
            .find(|&column| self.tab_stops[usize::from(column)])
            .unwrap_or(cols - 1)
    }

    /// Plain, printable transcript rows. Concealed cells are omitted from
    /// user-facing copy, search, and accessibility output.
    pub fn transcript_rows(&self) -> Vec<(String, bool)> {
        self.grid()
            .history_rows()
            .map(|row| {
                let mut text = String::new();
                for col in 0..self.grid().size().cols {
    ...
    /// Cursor position for DSR; DECOM makes the row relative to the margins.
    pub fn reported_cursor_position(&self) -> (u16, u16) {
        self.grid().reported_cursor_position()
    }

    /// DECSCUSR parameter, 0..=6. Zero is the default blinking block.
...
impl Screen {
    pub(crate) fn text(&mut self, c: char) {
        let c = self.map_character(c);
        if self.extend_grapheme(c) {
            return;
        }
        // A two-cell character cannot fit a one-column viewport. Replacement
        // keeps the grid valid and follows the same policy in both painters.
        let c = if self.grid().size().cols == 1 && c.width() == Some(2) {
            '\u{fffd}'
        } else {
            c
        };
    ...
    pub(crate) fn decsc(&mut self) {
        self.save_cursor();
        self.saved_charsets = (self.charsets, self.active_charset);
    }

    // ESC 8
    pub(crate) fn decrc(&mut self) {
        self.restore_cursor();
        (self.charsets, self.active_charset) = self.saved_charsets;
    }
    ...
    pub(crate) fn decset(&mut self, params: &vte::Params, mut unhandled: impl FnMut(&mut Self)) {
        for param in params {
            match param {
                ...
                [12] => {
                    self.cursor_style = self.cursor_style.saturating_sub(u16::from(
                        self.cursor_style > 0 && self.cursor_style % 2 == 0,
                    ))
                }
    ...
    pub(crate) fn decrst(&mut self, params: &vte::Params, mut unhandled: impl FnMut(&mut Self)) {
        for param in params {
            match param {
                ...
                [12] => {
                    self.cursor_style = if self.cursor_style == 0 {
                        2
                    } else {
                        self.cursor_style + self.cursor_style % 2
                    }
                }
    ...
    pub(crate) fn sgr(&mut self, params: &vte::Params, mut unhandled: impl FnMut(&mut Self)) {
        ...
        loop {
            match next_param!() {
                ...
                [21] => self.attrs.set_double_underline(),
                [5] => self.attrs.set_blink(true),
                [6] => self.attrs.set_rapid_blink(),
...
impl Screen {
    // ISO 2022 designation/invocation; xterm ctlseqs, VT100 character sets.
    pub(crate) fn designate(&mut self, slot: u8, charset: u8) {
        if matches!(charset, b'0' | b'A' | b'B') && (b'('..=b'+').contains(&slot) {
            self.charsets[usize::from(slot - b'(')] = charset;
        }
    }

    pub(crate) fn shift(&mut self, slot: usize) {
        self.active_charset = slot.min(3);
    }

    fn map_character(&self, c: char) -> char {
    ...
    pub(crate) fn tab_clear(&mut self, mode: u16) {
        match mode {
            0 => {
                let col = usize::from(self.grid().pos().col);
                if let Some(stop) = self.tab_stops.get_mut(col) {
    ...
    pub(crate) fn ansi_mode(&mut self, params: &vte::Params, enabled: bool) {
        for param in params {
            match param {
    ...
    fn extend_grapheme(&mut self, c: char) -> bool {
        ...
        let Some(mut pos) = self.last_printed else {
            return false;
        };
        let Some(cell) = self.grid().drawing_cell(pos) else {
            return false;
        };
        let previous = cell.contents();
        // The overwhelmingly common ASCII path does not allocate or segment.
        if previous.is_empty() || (c.is_ascii() && previous.is_ascii()) {
            return false;
        }
        let mut cluster = previous.to_owned();
        cluster.push(c);
        if cluster.graphemes(true).nth(1).is_some() {
            return false;
        }
        // A malicious, endless combining sequence must not grow the cell.
        if cluster.len() > 256 {
            return true;
        }
        let old_width = if cell.is_wide() { 2 } else { 1 };
        let width = (cluster.width() as u16).clamp(1, 2);
        let attrs = *cell.attrs();
        let size = self.grid().size();
        if width > size.cols {
            return true;
        }
        if pos.col + width > size.cols {
            if !self.auto_wrap {
                return true;
            }
            self.grid_mut()
                ...
                .unwrap()
                .erase(pos.col, attrs);
            self.grid_mut().col_wrap(width, true);
            pos = self.grid().pos();
        } else if old_width > width {
            if let Some(next) = self.grid_mut().drawing_cell_mut(crate::grid::Pos {
                row: pos.row,
                col: pos.col + 1,
            }) {
                next.clear(attrs);
        ...
        if width == 2 {
            ...
            self.grid_mut()
                .drawing_row_mut(pos.row)
                .unwrap()
                .clear_wide(next_pos.col);
            let next = self.grid_mut().drawing_cell_mut(next_pos).unwrap();
            next.clear(attrs);
            next.set_wide_continuation(true);
        }
        let first = self.grid_mut().drawing_cell_mut(pos).unwrap();
        first.clear(attrs);
        first.set_contents(&cluster, width == 2);
        self.grid_mut().col_set(pos.col);
