...
/// Per-image sixel sequence-cache diagnostics, read by TRust's `TRUST_DIAG_FRAME`.
///
/// A *build* is a full [`SlicedSixelData::to_sequence`] rebuild (`bands().join()`
/// — a whole-image byte-string allocation+copy, the per-image-per-draw cost the
/// cache exists to kill) on the RENDER thread; a *hit* reuses an `Arc<str>`
/// already built for the same slice key; a *prewarm* is a build moved OFF the
/// render thread (the encode thread, [`SlicedProtocol::prewarm_sixel_cache`]) so
/// a newly-appearing image's first draw is a hit, not a render-thread build. At
/// rest with on-screen sixels, hits should track widget renders and builds stay
/// near zero.
pub static SIXEL_SEQ_BUILDS: AtomicU64 = AtomicU64::new(0);
pub static SIXEL_SEQ_HITS: AtomicU64 = AtomicU64::new(0);
pub static SIXEL_SEQ_PREWARMS: AtomicU64 = AtomicU64::new(0);

/// Cache key for a built sixel sequence. A given `SlicedSixel`'s output is a pure
/// function of which bands it emits (`skip`/`drop`) and the target cell box
/// (`width`/`height`) — every other input is immutable owner data — so a 1-entry
/// memo keyed on this is always correct: at rest an on-screen image has one stable
/// key and hits every frame; scrolling changes `skip`/`drop` and forces a single
/// rebuild (identical to the un-cached cost).
...
/// The memoized sixel byte-string for the current slice, carried *beside* (not
/// inside) the self-referential [`SlicedSixel`] so the cache holds fully owned
/// data without fighting the `self_cell` borrow. `Arc` (not `Rc`) keeps
/// [`SlicedProtocol`] `Send`: it is built on a blocking task and moved to the
/// render thread over a channel.
pub type SixelSeqCache = RefCell<Option<(SeqKey, Arc<str>)>>;
...
impl Widget for SlicedImage<'_> {
        ...
        Self: Sized,
    {
        ...
        match &self.sliced_protocol {
            ...
            SlicedProtocol::Sixel(sliced_sixel, cache) => {
                let sliced = sliced_sixel.borrow_dependent();
                sliced.render_cached(image_area, buf, skip_line_count, drop_line_count, cache);
            }
...
pub enum SlicedProtocol {
    ...
    ///
    /// The second field memoizes the rendered byte-string for the current slice
    /// (see [`SixelSeqCache`]); without it an at-rest full-frame redraw rebuilds
    /// every visible image's whole sixel string each draw.
    Sixel(SlicedSixel, SixelSeqCache),
...
impl SlicedProtocol {
    ...
    pub fn new_with_resize(
        ...
        match picker.protocol_type() {
            ...
            ProtocolType::Sixel => {
                ...
                Ok(SlicedProtocol::Sixel(sliced, RefCell::new(None)))
            }
    ...
    pub fn size(&self) -> Size {
        match self {
            ...
            SlicedProtocol::Sixel(sixel_slice, _) => sixel_slice.borrow_owner().size(),
        }
    }

    /// Pre-build the *at-rest* slice (the image fully visible and unscrolled:
    /// `skip = drop = 0`, box = the encoded size) into the cache, so the first
    /// on-screen draw is a cache HIT instead of a render-thread
    /// [`to_sequence`](SlicedSixelData::to_sequence) build. Intended to run on the
    /// encode thread right after construction — it moves that one build off the
    /// render thread for every newly-appearing image (a streaming chat's steady
    /// flow of emotes). A no-op for non-sixel protocols.
    ///
    /// Correctness: the at-rest key is exactly what a fully-visible render
    /// computes, so the prewarmed entry hits. If the image instead first appears
    /// partly scrolled, the entry misses and the render path builds the real
    /// slice as before — the 1-entry memo is simply replaced. Never wrong; at
    /// worst the same cost as no prewarm.
    pub fn prewarm_sixel_cache(&self) {
        if let SlicedProtocol::Sixel(sliced, cache) = self {
            let size = sliced.borrow_owner().size();
            let key = SeqKey {
                skip_line_count: 0,
                drop_line_count: 0,
                width: size.width,
            ...
            let seq: Arc<str> =
                Arc::from(sliced.borrow_dependent().to_sequence(0, 0, size.width, size.height));
            *cache.borrow_mut() = Some((key, seq));
            SIXEL_SEQ_PREWARMS.fetch_add(1, Ordering::Relaxed);
        }
    }
}
...
mod sixel_slice {
    ...
    impl<'a> SlicedSixelData<'a> {
        ...
        /// expensive `to_sequence` rebuild (`bands().join()`) runs only on a
        /// cache miss; at rest an on-screen sixel hits every frame, so a
        /// full-frame redraw no longer rebuilds every visible image's whole
        /// sixel string. The cache lives in `SlicedProtocol::Sixel` (beside the
        /// self-referential owner), so it is passed in by reference here.
        pub fn render_cached(
            ...
            skip_line_count: usize,
            drop_line_count: usize,
            cache: &super::SixelSeqCache,
        ) {
            if self.size.width > area.width {
                return;
            }

            let key = super::SeqKey {
                skip_line_count,
                drop_line_count,
                width: area.width,
                height: area.height,
            ...
            let data = {
                ...
                match slot.as_ref() {
                    Some((cached_key, seq)) if *cached_key == key => {
                        super::SIXEL_SEQ_HITS.fetch_add(1, Ordering::Relaxed);
                        Arc::clone(seq)
                    }
                    _ => {
                        let seq: Arc<str> = Arc::from(self.to_sequence(
                            skip_line_count,
                            drop_line_count,
                            area.width,
                            area.height,
                        ));
                        super::SIXEL_SEQ_BUILDS.fetch_add(1, Ordering::Relaxed);
                        *slot = Some((key, Arc::clone(&seq)));
                        seq
                    }
                }
            };
    ...
    mod tests {
        ...
        fn test_bands_from_image() {
            ...
            let SlicedProtocol::Sixel(proto, _) = SlicedProtocol::new(&picker, image, None).unwrap()
            else {
