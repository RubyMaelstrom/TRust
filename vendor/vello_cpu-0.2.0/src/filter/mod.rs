...
/// Filter Effects 1 #feColorMatrixElement: operate on straight color, clamp
/// each result, then premultiply for compositing. CSS supplies sRGB matrices.
fn color_matrix(matrix: &[f32; 20], pixmap: &mut Pixmap) {
    for pixel in pixmap.data_mut() {
        let alpha = f32::from(pixel.a);
        let input = if alpha > 0.0 {
            [f32::from(pixel.r) / alpha, f32::from(pixel.g) / alpha,
             f32::from(pixel.b) / alpha, alpha / 255.0]
        } else { [0.0; 4] };
        let output: [f32; 4] = core::array::from_fn(|row| {
            let m = &matrix[row * 5..row * 5 + 5];
            (m[0]*input[0] + m[1]*input[1] + m[2]*input[2] + m[3]*input[3] + m[4]).clamp(0.0, 1.0)
        });
        *pixel = vello_common::color::PremulRgba8 {
            r:(output[0]*output[3]*255.0 + 0.5) as u8,
            g:(output[1]*output[3]*255.0 + 0.5) as u8,
            b:(output[2]*output[3]*255.0 + 0.5) as u8,
            a:(output[3]*255.0 + 0.5) as u8,
        };
    }
}
...
pub(crate) fn filter_lowp(
    ...
    match prepared_filter {
        PreparedFilter::ColorMatrix(matrix) => color_matrix(&matrix, pixmap),
