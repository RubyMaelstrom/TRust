...
struct GcConfig {
    /// The current heap size (bytes) at which the next collection triggers.
    /// Re-sized after every collection (see `Allocator::manage_state`).
    threshold: usize,
    /// The minimum the threshold is ever sized to. The trigger never drops
    /// below this, so a tiny live set still allows cheap, cache-friendly
    /// frequent collection.
    floor: usize,
    /// After a collection, the next threshold is `max(floor, live *
    /// growth_percent / 100)` — i.e. the GC lets the live ("surviving") set
    /// grow by this percentage in garbage before collecting again. This
    /// keeps GC overhead a roughly fixed fraction of allocation regardless
    /// of live-set size, and (unlike a grow-only policy) lets the threshold
    /// shrink back when a page's live set drops.
    growth_percent: usize,
    /// Once the live set exceeds this, `big_growth_percent` is used instead
    /// of `growth_percent`. Rationale: while the live set fits in cache,
    /// frequent collection is cheap (a full mark is small) AND keeps the
    /// working set cache-resident, so a low budget wins — raising it
    /// measurably regresses allocation-heavy moderate pages. Once the live
    /// set itself exceeds cache, marking is already out-of-cache no matter
    /// what, so the only thing that helps is doing FEWER (expensive) marks —
    /// i.e. a big budget. So we keep small heaps tight and only let large
    /// ones run loose.
    big_live: usize,
    /// `growth_percent` used when the live set exceeds `big_live`.
    big_growth_percent: usize,
}

impl Default for GcConfig {
    fn default() -> Self {
        Self {
            // Boa's historical default was a 1 MiB grow-only threshold with
            // a 70%-used grow trigger (~1.43x). That thrashes full mark-sweeps
            // on bundles whose live set grows large (it takes ~log_1.43 steps
            // — ~19 collections — to chase a 20 MiB live set up from 1 MiB).
            // TRust resizes to a multiple of the live set instead; see the
            // field docs and `Allocator::manage_state`.
            threshold: 1_048_576,
            floor: 1_048_576,
            growth_percent: 143,
            big_live: 16 * 1_048_576,
            big_growth_percent: 400,
        }
    }
}
...
struct GcRuntimeData {
    ...
    /// Cumulative wall time spent in `Collector::collect` (instrumentation
...
/// time, bytes currently tracked as allocated). TRust reads this around a
/// bundle load to see how much of the time is the garbage collector.
#[must_use]
pub fn gc_profile() -> (usize, Duration, usize) {
    BOA_GC.with(|st| {
        let gc = st.borrow();
        (
...
/// Set the GC floor (bytes): the minimum heap size the collection trigger
/// is ever sized to, and the initial threshold. The trigger then tracks the
/// live set (see `set_gc_growth_percent`) but never drops below this.
pub fn set_gc_threshold(floor: usize) {
    BOA_GC.with(|st| {
        let mut gc = st.borrow_mut();
        gc.config.floor = floor.max(1);
        gc.config.threshold = gc.config.threshold.max(gc.config.floor);
    });
}

/// Set how aggressively the threshold grows relative to the live set after
/// each collection: the next trigger is `max(floor, live * percent / 100)`.
/// 100 = collect again as soon as the heap re-doubles the survivors;
/// 400 (TRust's default) = let 3x the live set accumulate as garbage first,
/// which keeps a large live set from triggering frequent (expensive) marks.
pub fn set_gc_growth_percent(percent: usize) {
    BOA_GC.with(|st| {
        st.borrow_mut().config.growth_percent = percent.max(100);
    });
}

/// Set the large-heap growth policy: once the live set exceeds `big_live`
/// bytes, the threshold grows by `percent` instead of the base growth (see
/// the `big_live` field docs for why a large live set wants a looser budget).
pub fn set_gc_big_growth(big_live: usize, percent: usize) {
    BOA_GC.with(|st| {
        let mut gc = st.borrow_mut();
        gc.config.big_live = big_live;
        gc.config.big_growth_percent = percent.max(100);
    });
}
...
impl Allocator {
    ...
    fn manage_state(gc: &mut BoaGc) {
        if gc.runtime.bytes_allocated > gc.config.threshold {
            ...
            // Size the next threshold to a multiple of the surviving (live)
            // set, clamped to the floor. A big live heap therefore gets a
            // big garbage budget (few expensive full marks) while a small
            // one stays small (cheap, cache-friendly frequent collection).
            // This both grows and shrinks, so a page whose live set falls
            // doesn't keep an oversized threshold. The multiple steps up once
            // the live set exceeds `big_live` (past cache, locality is lost
            // so fewer marks is all that helps) — see the field docs.
            let live = gc.runtime.bytes_allocated;
            let growth = if live > gc.config.big_live {
                gc.config.big_growth_percent
            } else {
                gc.config.growth_percent
            };
            gc.config.threshold = live
                .saturating_mul(growth)
                .checked_div(100)
                .unwrap_or(gc.config.floor)
                .max(gc.config.floor);
        }
    }
}
...
impl Collector {
    ...
    fn collect(gc: &mut BoaGc) {
        let __collect_start = Instant::now();
        ...
        gc.runtime.total_collect_time += __collect_start.elapsed();
    }
