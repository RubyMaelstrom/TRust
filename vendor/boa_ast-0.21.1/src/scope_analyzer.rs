...
/// Optimize scope indicies for a dynamically-compiled function (`new Function`,
/// `Function.prototype.constructor`).
///
/// This is the same pass as [`optimize_scope_indicies`], except it reserves a
/// stack slot for the function's own environment. The dynamic-function path
/// always pushes a function environment (`FunctionCompiler::force_function_scope`),
/// even when escape analysis would otherwise elide it; if the index pass elides
/// it here, every binding nested under the function — and anything a `with`
/// inside it leaves on the environment stack (Vue's `v-for` render closure) —
/// resolves one environment too low at runtime. Static scripts/modules never
/// force the scope, so they keep using [`optimize_scope_indicies`].
pub(crate) fn optimize_function_scope_indicies<'a, N>(node: &'a mut N, scope: &Scope)
where
    &'a mut N: Into<NodeRefMut<'a>>,
{
    let mut visitor = ScopeIndexVisitor {
...
struct ScopeIndexVisitor {
    index: u32,
    /// Force a function environment for the next function visited (the root of
    /// a dynamically-compiled function). Consumed on entry so nested functions
    /// keep the normal escape-analysis decision.
...
impl ScopeIndexVisitor {
    fn visit_function_like(
        ...
        // Only the root of a dynamically-compiled function forces its scope;
        // take the flag so nested functions fall back to escape analysis.
        let force_function_scope = std::mem::take(&mut self.force_function_scope);
        let index = self.index;
        ...
        if contains_direct_eval || !scopes.function_scope().all_bindings_local() {
            ...
            self.index += 1;
        } else if force_function_scope {
            // The dynamic-function path always pushes a function environment, so
            // reserve its stack slot even though the bindings are all local.
