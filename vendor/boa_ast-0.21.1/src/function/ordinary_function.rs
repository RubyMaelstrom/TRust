...
use crate::{
    ...
    scope_analyzer::{analyze_binding_escapes, collect_bindings, optimize_function_scope_indicies},
...
impl FunctionExpression {
    ...
    pub fn analyze_scope(
        ...
        // Reconcile compile-time scope indices with the environment stack the
        // VM actually pushes — static scripts/modules run this in their own
        // `analyze_scope`, but dynamic functions skipped it, so `with` bodies
        // and the closures inside them resolved bindings one environment off.
        optimize_function_scope_indicies(self, scope);
        Ok(())
    }
}
