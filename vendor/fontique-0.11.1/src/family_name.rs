...
impl NameKey {
    fn from_str(s: &str) -> Self {
        ...
        let mut buf = [0_u8; 4];
        #[cfg(feature = "std")]
        let chars = s.case_fold();
        #[cfg(not(feature = "std"))]
        let chars = s.chars().flat_map(char::to_lowercase);
        for ch in chars {
            res.data
...
mod tests {
    ...
    #[test]
    fn family_names_use_unicode_default_caseless_matching() {
        let mut names = FamilyNameMap::default();
        let inserted = names.get_or_insert("Straße Gothic");
        assert_eq!(names.get("STRASSE GOTHIC").map(|name| name.id()), Some(inserted.id()));
    }
}
