...
impl HostDefined {
    ...
    // TRust fork: newer hashbrown renames `get_many_mut` -> `get_disjoint_mut`
    // BUT changes the return shape (`Option<[&mut V; N]>` -> `[Option<&mut V>; N]`),
    // so it is not a drop-in swap for `mut_ref_from_anys`. Keep the old API.
    #[allow(deprecated)]
    #[track_caller]
