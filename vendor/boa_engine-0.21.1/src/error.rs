...
impl JsError {
    ...
    pub fn to_opaque(&self, context: &mut Context) -> JsValue {
        ...
        // TRust fork: Boa captures a full backtrace onto this `JsError`
        // wrapper in the VM throw path, and `Display` already renders it —
        // but materializing the JS object dropped it, so page JS never had a
        // `.stack`. That was the root of the "opaque, stackless rejection"
