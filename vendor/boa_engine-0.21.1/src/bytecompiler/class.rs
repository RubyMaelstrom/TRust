...
impl ByteCompiler<'_> {
    ...
    pub(crate) fn compile_class(&mut self, class: ClassSpec<'_>, dst: Option<&Register>) {
        ...
        if let Some(expr) = &class.constructor {
            // TRust fork (trap #6 fix): materialize the function scope at
            // runtime under the SAME rule regular functions use (see
            // `FunctionCompiler::compile`), instead of unconditionally.
            // `boa_parser` assigns binding scope-indices assuming an
            // all-local, non-required function scope is ELIDED; the old
            // unconditional `push_scope` inserted an extra runtime env, so a
            // `const`/`let` in the constructor body captured by a closure
            // resolved to the empty function env (0 slots) instead of its own
            // lexical env -> the define-opcode out-of-bounds panic. Pushing
            // only when `boa_parser` also expects it keeps compile-time
            // locators and runtime environments aligned.
            let scopes = expr.scopes();
