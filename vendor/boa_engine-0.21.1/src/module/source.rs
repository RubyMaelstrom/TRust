...
impl std::fmt::Debug for SourceTextModule {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // TRust fork: `loaded_modules` / `async_parent_modules` hold `Module`s
        // and the graph is cyclic, so the upstream impl recursed forever. It
        // also borrowed `loaded_modules`, which the failing assert in this
        // file holds mutably -> a borrow panic *during* panic formatting.
        // Print only the non-recursive code identity; never touch the
        // module-valued, possibly-borrowed cells from Debug.
        f.debug_struct("SourceTextModule")
            .field("path", &self.code.path)
            .field("requested_modules", &self.code.requested_modules)
