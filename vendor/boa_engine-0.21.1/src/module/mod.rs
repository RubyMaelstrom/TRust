...
impl std::fmt::Debug for Module {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        // TRust fork: a module's graph (`kind` -> SourceTextModule ->
        // `loaded_modules`) is CYCLIC for any page that does a dynamic
        // `import()` of a statically-imported specifier. The upstream impl
        // recursed through `kind`, so formatting a module while building a
        // panic message (the `assert_eq!(entry, loaded)` in source.rs)
        // overflowed the native stack -> an UNCATCHABLE abort that took the
        // whole app down. Print stable, non-recursive identity instead so
        // that assert degrades to an ordinary, catch_unwind-able panic.
        f.debug_struct("Module")
            .field("addr", &std::ptr::from_ref::<ModuleRepr>(&self.inner))
