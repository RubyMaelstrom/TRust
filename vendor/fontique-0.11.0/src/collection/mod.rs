...
impl CommonData {
    ...
    fn load_fonts_from_paths(&mut self, paths: impl IntoIterator<Item = impl AsRef<Path>>) {
        ...
        let mut source_paths = SourcePathMap::default();
        ...
        crate::scan::scan_paths(paths, 16, |scanned_font| {
            ...
            if !crate::scan::all_names(
                &scanned_font.name_table,
                NameId::TYPOGRAPHIC_FAMILY_NAME,
                &mut name_pool,
                &mut names,
            ) && !crate::scan::all_names(
                &scanned_font.name_table,
                NameId::FAMILY_NAME,
                &mut name_pool,
                &mut names,
            ...
            let Some(font) = FontInfo::from_font_ref(
                &scanned_font.font,
                source_paths.get_or_insert(path),
                scanned_font.index,
            ) else {
                return;
            };
            let Some((first_name, other_names)) = names.split_first() else {
                return;
            };
            let family_name = self.family_names.get_or_insert(first_name);
            for other_name in other_names {
                self.family_names.add_alias(family_name.id(), other_name);
            }
            families
                .entry(family_name.id())
                .or_insert_with(|| (family_name, Vec::new()))
                .1
                .push(font);
        });
    ...
    fn register_font_impl(
        ...
        super::scan::scan_memory(font_data, |scanned_font| {
            name_pool.append(names);
            let overridden = info_override.and_then(|info| info.family_name);
            if let Some(override_name) = overridden {
                let mut name = name_pool.pop().unwrap_or_default();
                name.clear();
                name.push_str(override_name);
                names.push(name);
            } else if !crate::scan::all_names(
                &scanned_font.name_table,
                NameId::TYPOGRAPHIC_FAMILY_NAME,
                name_pool,
                names,
            ...
            let name = self.family_names.get_or_insert(first_name);
            if overridden.is_none() {
                for other_name in other_names {
                    self.family_names.add_alias(name.id(), other_name);
                }
            }
            families
                .entry(name.id())
