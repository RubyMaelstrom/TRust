    ...
    TagSize: crate::TagSize,
{
    fn encrypt_inout_detached(
        ...
        self.encrypt_inout_detached_with_nonce(nonce, associated_data, buffer)
    }
    ...
    fn decrypt_inout_detached(
        ...
        self.decrypt_inout_detached_with_nonce(nonce, associated_data, buffer, tag)
    }
}
...
where
    ...
    TagSize: crate::TagSize,
{
    /// Encrypt with a runtime-sized IV, as required by Web Crypto's
    /// `AesGcmParams`. The caller must ensure IV uniqueness for this key.
    /// NIST SP 800-38D §5.2.1.1 requires a nonempty IV fitting a 64-bit bit length.
    pub fn encrypt_inout_detached_with_nonce(
        &self,
        nonce: &[u8],
        associated_data: &[u8],
        ...
        if nonce.is_empty()
            || nonce.len() as u64 > u64::MAX / 8
            || buffer.len() as u64 > P_MAX
    ...
    /// Authenticate and decrypt with a runtime-sized IV. A failed
    /// authentication leaves the output buffer untouched.
    pub fn decrypt_inout_detached_with_nonce(
        &self,
        nonce: &[u8],
        associated_data: &[u8],
        buffer: InOutBuf<'_, '_, u8>,
