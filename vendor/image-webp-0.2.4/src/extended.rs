...
mod tests {
    ...
    fn dispose_clear_subframe_rgb_frame() {
        ...
        let (prev_x, prev_y, prev_w, prev_h) = (2u32, 2u32, 4u32, 4u32);
        let (frame_w, frame_h) = (4u32, 4u32);
        ...
        for y in prev_y as usize..(prev_y + prev_h) as usize {
            for x in prev_x as usize..(prev_x + prev_w) as usize {
                let pixel = &canvas[y * stride + x * 4..][..4];
        ...
        assert_eq!(&canvas[7 * 4..7 * 4 + 4], [0xAA, 0xAA, 0xAA, 0xAA]);
    }
}
