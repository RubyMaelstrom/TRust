...
impl<'a, R: ReadChar> Parser<'a, R> {
    ...
    /// Parse the full input as an [ECMAScript Script][spec] into the boa AST,
    /// **without** running scope analysis.
    ///
    /// This is the raw-parse half of [`Self::parse_script_with_source`]: it
    /// builds the AST and source text but skips `analyze_scope`, so the result
    /// is not yet ready for bytecode compilation. It exists so the lexer/parser
    /// (the bulk of the "parse" phase) can run on a worker thread with a private
    /// [`Interner`], off the page's `Context`. The caller then runs
    /// [`boa_ast::Script::analyze_scope`] against the real (shared) scope on the
    /// owning thread before compiling. Unlike the public entries this needs no
    /// [`Scope`] — scope analysis is the caller's responsibility — and the
    /// returned AST is `Send` (it holds no `JsString`/`Rc`, only `Sym`s into the
    /// interner), which is what lets it cross to the page thread paired with its
    /// interner.
    ///
    /// # Errors
    ///
