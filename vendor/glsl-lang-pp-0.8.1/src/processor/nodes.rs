...
impl Line {
    pub fn parse(
        ...
        // TRust: this recursive evaluator runs before the browser's GLSL lexer.
        let mut depth = 0usize;
        let mut count = 0usize;
        for token in subs_events.iter().filter_map(Event::as_token) {
            count += 1;
            ...
            if count > 1024 || depth > 64 {
                return Err(LineError::ResourceLimit);
            }
        }
...
#[derive(Debug, Clone, PartialEq, Eq, Error)]
pub enum LineError {
    #[error("line expression resource limit exceeded")]
    ResourceLimit,
...
pub enum IfEvalError {
    ...
    #[error("conditional expression resource limit exceeded")]
    ResourceLimit,
...
fn eval_inner(
    ...
    for token in subs_events.iter().filter_map(Event::as_token) {
        ...
        if count > 1024 || depth > 64 {
            return (true, Some(IfEvalError::ResourceLimit));
        }
    }
