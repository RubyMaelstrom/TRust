...
// TRust: the public lexer token limit runs after this eager expansion. Bound
// work and storage here as well, including repeated arguments and token pasting.
const MAX_EXPANSION_TOKENS: usize = 100_000;
const MAX_EXPANSION_DEPTH: usize = 64;
const MAX_PASTE_BYTES: usize = 1024 * 1024;

fn expansion_limit(range: NodeSpan, location: &ExpandLocation) -> Vec<Event> {
    vec![Event::error(
...
impl Definition {
    ...
    fn concat_node_to_tokens<T: TokenLike>(
        ...
        let mut pasted_bytes = 0usize;
        let mut pasted_tokens = 0usize;
        ...
        loop {
            ...
            pasted_bytes = pasted_bytes.saturating_add(current_text.to_string().len());
            pasted_tokens += 1;
            if pasted_bytes > MAX_PASTE_BYTES || pasted_tokens > MAX_EXPANSION_TOKENS {
                return vec![OutputToken::new_error(entire_range)];
            }
    ...
    fn substitute_define_object(
        ...
        for node_or_token in object.body().children_with_tokens() {
            if tokens.len() >= MAX_EXPANSION_TOKENS {
                return expansion_limit(entire_range, location);
            }
            match node_or_token {
                ...
                NodeOrToken::Token(token) => {
                    ...
                    if kind == IDENT_KW {
                        ...
                        if let Some(value) = args.get(text.try_as_str().unwrap()) {
                            ...
                            if value.len() > MAX_EXPANSION_TOKENS.saturating_sub(tokens.len()) {
                                return expansion_limit(entire_range, location);
                            }
...
impl<'d> MacroInvocation<'d> {
    ...
    pub fn substitute_vec(
        ...
        let mut remaining = MAX_EXPANSION_TOKENS;
    ...
    fn substitute_vec_inner(
        ...
        if tokens.len() > *remaining || subs_stack.len() > MAX_EXPANSION_DEPTH {
            *remaining = 0;
            return tokens.first().map_or_else(Vec::new, |t| {
                expansion_limit(range.unwrap_or_else(|| t.text_range()), location)
            });
        }
    ...
    pub fn substitute(
        ...
        let mut subs_stack = HashSet::new();
        let mut remaining = MAX_EXPANSION_TOKENS;
        self.substitute_inner(current_state, location, &mut subs_stack, &mut remaining)
    }
    ...
    fn substitute_inner(
        ...
        if *remaining == 0 || subs_stack.len() >= MAX_EXPANSION_DEPTH {
            *remaining = 0;
            return expansion_limit(self.range, location);
        }
