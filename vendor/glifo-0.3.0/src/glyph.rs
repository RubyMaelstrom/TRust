...
impl<'a, B> GlyphRunBuilder<'a, B> {
    ...
    /// Select the policy used when hinting is enabled. This does not change
    /// glyph positions or advances supplied by the shaper.
    pub fn hinting_mode(mut self, mode: HintingMode) -> Self {
        self.run.hinting_mode = mode;
        self
    }
...
/// Raster hinting policies. Both preserve the shaper's linear advances.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Hash)]
pub enum HintingMode {
    /// Prefer native font instructions, using vertical-only LCD hinting.
    #[default]
    Vertical,
    /// FreeType-style slight hinting: automatically fit TrueType outlines
    /// vertically while retaining fractional sizes. CFF/CFF2 and fonts that
    /// require their native interpreter retain that interpreter.
    Light,
}
...
impl<'a> OutlineCacheSession<'a> {
    ...
    pub(crate) fn get_or_insert(
        ...
        let key = OutlineKey {
            ...
            hint: hinting_instance.map(|_| hinting_mode),
        };
...
impl HintKey<'_> {
    fn options(&self) -> HintingOptions {
        if self.mode == HintingMode::Vertical {
            return HINTING_OPTIONS;
        }
        // FreeType FT_LOAD_TARGET_LIGHT selects native vertical hinting for
        // PostScript and the auto-hinter for ordinary TrueType outlines.
        // Hint-reliant fonts are the FT_FACE_FLAG_TRICKY exception: Skrifa
        // requires the interpreter and a monochrome target for these.
        if self.outlines.require_interpreter() {
